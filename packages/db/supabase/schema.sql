-- ============================================================
-- VelvetMatching — Supabase PostgreSQL Schema
...
create table public.transactions (
  id             uuid       primary key default uuid_generate_v4(),
  user_id        uuid       not null references public.users(id) on delete cascade,
  amount         int        not null,  -- in smallest currency unit (cents)
...
-- ── Contacts (protected by RLS — only revealed after paid unlock) ─────────────
...
-- Auto-update updated_at on advertisement changes
...
-- Advertisements: public read, own write
create policy "ads_select_all" on public.advertisements
...
-- Contacts: reveal only after paid unlock, or to own advertiser
create policy "contacts_select_if_unlocked" on public.contacts
  for select using (
    exists (
      select 1
      from public.unlocks u
      join public.transactions t on t.id = u.tx_id
      where u.ad_id     = contacts.ad_id
        and u.client_id = auth.uid()
        and t.status    = 'paid'
    )
  );

create policy "contacts_select_own_ad" on public.contacts
  for select using (
    exists (
      select 1 from public.advertisements a
      where a.id = contacts.ad_id and a.user_id = auth.uid()
    )
  );

create policy "contacts_insert_own_ad" on public.contacts
...
create policy "contacts_update_own_ad" on public.contacts
  for update using (
    exists (
      select 1 from public.advertisements a
      where a.id = contacts.ad_id and a.user_id = auth.uid()
...
-- Unlocks: client inserts and reads own
create policy "unlocks_insert_client" on public.unlocks
...
-- Categories + ad_categories: public read
create policy "categories_select_all" on public.categories
  for select using (true);

create policy "ad_categories_select_all" on public.ad_categories
  for select using (true);
...
-- AI screenings: own advertiser reads
create policy "ai_screenings_select_own" on public.ai_screenings
  for select using (
    exists (
      select 1 from public.advertisements a
      where a.id = ai_screenings.ad_id and a.user_id = auth.uid()
    )
  );
