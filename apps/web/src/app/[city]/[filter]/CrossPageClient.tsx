...
interface CrossPageClientProps {
  cityNameEn: string
...
export default function CrossPageClient({ cityNameEn, searchParams, initialProfiles, initialTotal }: CrossPageClientProps) {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const qs = new URLSearchParams({ ...searchParams, city: cityNameEn, page: String(nextPage) })
  ...
  }, [searchParams, cityNameEn, page])

  if (profiles.length === 0) {
    return (
      <div className="text-center py-24 bg-white/[0.02] rounded-3xl border border-white/[0.04]">
        <div className="text-5xl mb-4 opacity-30">🔍</div>
        <p className="text-white/30 text-sm font-bold mb-5">No profiles found for this combination yet</p>
        <a
          href="/"
